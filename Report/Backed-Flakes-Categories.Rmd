...
```{r PCA on procrustes, eval=FALSE}
...
```
```{r Join PC and manually recorded attributes, eval=FALSE}
...
```{r Data Pre processing, eval=FALSE}
#### Pre processing data
...
```{r Set formula and validation, eval=FALSE}
...
As mentioned previously, this is a unbalanced data set. Balancing to train the models correctly is done using the `groupdata2` package using the parameter **"mean"** for size. This means that each group will be up or down sampled to the result of dividing data set size between number of groups . 
