---
title: "A geometric morphometric approach to testing discrete categories of backed
  flakes from recurrent centripetal core reduction".
...
PC from geometric morphometrics are hard to interpret as a result of reducing dimensionality through the identification of linear combinations of variables. In some cases, the problem of interpretability can be overcome by using manual measurements as independent variables in a multiple linear regression to predict a given PC [@bustos-perez_combining_2022]. In addition to geometric morphometrics, the following attributes were recorded for each of the flakes:    
  ...
  * **Relative amount of cortex present at the dorsal face:** recorded according to its extension on the dorsal surface of the flake, with categories being as follows: 0 (no cortex), 1 (nearly 25% covered by cortex), 2 (nearly 50% covered by cortex), 3 (nearly 75% covered by cortex), and 4 (nearly all of the surface covered by cortex). This variable is employed to evaluate the distribution among the experimental assemblage (Figure 2).   
...
From the previous measures, the following variables are obtained:   
  ...
  * **Carenated index:** width or length (the one with the lowest value) divided by maximum thickness.    
  * **Ratio of width to thickness:** flake width divided by maximum thickness.   

These measures and derived ratios are not employed as inputs for the Machine Learning models, but to explore the meaning of the PC through multiple linear regressions. However, many manual measurements do not capture changes in morphology, thus running the risk of having significant PC unexplained. To overcome this, Geometric Morphometrics approaches employ different types of visualizations to observe shape and changes in shape from one group to other [@klingenberg_visualizations_2013]. The present work employs a visualization of landmarks points according to an average shape. Given a significant PC for discrimination, artifacts are grouped into four groups. First, artifacts with PC values below the 1st quantile; second, artifacts with PC values between values of the 1st and 2nd quantiles; third, artifacts with PC values between values of the 2nd and 3rd quantile; and finally, artifacts with PC values higher than the 3rd quantile. Then, for each of these groups, the mean shape is calculated and visualized using a magnification of 2 to better detect changes in shape.    

### 2.3	Machine learning and resampling techniques    

The present work tests ten machine learning models for the classification of flake categories.    
  ...
  * **Random forest:** Random forests are made of decision trees. Each tree is grown from a random sample of the data and variables, allowing for each tree to grow differently and better reflect the complexity of the data [@breiman_random_2001].    
  * **Generalized Boosted Model:** [@greenwell_package_2019; @ridgeway_generalized_2007] which implements gradient boosted machine [@friedman_greedy_2001; @friedman_stochastic_2002], allowing for the detection of learning deficiencies and increase model accuracy for a set of random forests.    
  * **Supported vector machines (SVM):** fits hyperplanes into a multidimensional space with the objective of creating homogeneous partitions [@cortes_support-vector_1995; @frey_letter_1991]. The present study tests SVM with linear, radial, and polynomial kernels. 
...
The following code performs PCA on the rotated landmarks and extracts the PC values of each case. Additionally, ID's of each case are added as an additional variable and employed in a `left join()` with the attribute dataset.  
...
```
```{r Join PC and manually recorded attributes}
# Left joined with the attribute database
PCA_Coord <- left_join(PCA_Coord, Att, by = "ID")
```

Prior to model training it is necessary to preprocess the data. Labels of each artifact are changed using the `case_when()` function and new variable is set as factor with corresponding labels.   
...
Formula and validation (k fold corss validation) are set prior to model training.   
...
As mentioned previously, this is a unbalanced data set. Balancing to train the models correctly is done using the `groupdata2` package using the parameter **"mean"** for size. This means that each group will be up or down sampled to the result of dividing data set size between number of groups (`r nrow(PCA_Coord)/3`). 
...
```{r Results from up and down sampling, fig.width=10, fig.height=5.5}
...
```{r Violin and box plots, fig.width=9, fig.height=4}
