...
```
    ...
    ## [[1]]
    ##  [1] "Morpho"    "knitr"     "ranger"    "caret"     "lattice"   "forcats"   "stringr"  
    ##  [8] "dplyr"     "purrr"     "readr"     "tidyr"     "tibble"    "ggplot2"   "tidyverse"
...
PC from geometric morphometrics are hard to interpret as a result of
reducing dimensionality through the identification of linear
combinations of variables. In some cases, the problem of
interpretability can be overcome by using manual measurements as
independent variables in a multiple linear regression to predict a given
PC ([Bustos-Pérez et al. 2022a](#ref-bustos-perez_combining_2022)). In
...
multiple linear regressions. However, many manual measurements do not
capture changes in morphology, thus running the risk of having
significant PC unexplained. To overcome this, Geometric Morphometrics
approaches employ different types of visualizations to observe shape and
changes in shape from one group to other ([Klingenberg
2013](#ref-klingenberg_visualizations_2013)). The present work employs a
visualization of landmarks points according to an average shape. Given a
significant PC for discrimination, artifacts are grouped into four
groups. First, artifacts with PC values below the 1st quantile; second,
artifacts with PC values between values of the 1st and 2nd quantiles;
third, artifacts with PC values between values of the 2nd and 3rd
quantile; and finally, artifacts with PC values higher than the 3rd
quantile. Then, for each of these groups, the mean shape is calculated
and visualized using a magnification of 2 to better detect changes in
shape.
...
2022a](#ref-bustos-perez_combining_2022),
[2022b](#ref-bustos-perez_research_2022)).
...
([Bustos-Pérez et al. 2022a](#ref-bustos-perez_combining_2022)). Because
...
differentiate between backed products from discoidal and Levallois
